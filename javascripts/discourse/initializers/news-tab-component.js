...
export default apiInitializer("1.8.0", (api) => {
  ...
  function renderNews(container, tabKey = "news") {
    ...
    const items = filteredNewsItems
      .map((item, index) => {
        ...
        return `<li class="news-item"><hr><div class="news-title"><a href="${url}" target="_blank" referrerpolicy="no-referrer" rel="noreferrer">${title}</a></div>${meta}${description}</li>`;
      })
      .join("");
  ...
  function fetchNews(tabKey = "news", container, { force = false } = {}) {
    ...
    if (container && !feedState.loading) {
      container.innerHTML = `<div class="news-empty">${loadingMessage}</div>${loadingDogDiv}`;
    }
